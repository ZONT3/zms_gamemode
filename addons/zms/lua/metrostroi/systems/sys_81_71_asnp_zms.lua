...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_71_ASNP_ZMS")
TRAIN_SYSTEM.DontAccelerateSimulation = true
function TRAIN_SYSTEM:Initialize()
    self.LineOut = 0
    self.TriggerNames = {"R_ASNPMenu", "R_ASNPUp", "R_ASNPDown", "R_ASNPOn", "R_Program1", "R_Program2", "R_Program1H", "R_Program2H", "R_ASNPPath", "R_Radio"}
    --R_Announcer
    --R_Line
    self.Triggers = {}
    self.State = 0
    ...
    if not self.Train.R_ASNPOn then
        ...
        self.Train:LoadSystem("R_ASNPPath", "Relay", "Switch", {
...
if CLIENT then
    ...
    function TRAIN_SYSTEM:PrintText(x, y, text, inverse)
        ...
        for i = 1, #str do
            ...
            if inverse then
                draw.SimpleText(string.char(0x7f), "Metrostroi_ASNP", (x + i) * 20.5 + 5, y * 40 + 40, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
                draw.SimpleText(char, "Metrostroi_ASNP", (x + i) * 20.5 + 5, y * 40 + 40, Color(140, 190, 0, 150), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            else
                draw.SimpleText(char, "Metrostroi_ASNP", (x + i) * 20.5 + 5, y * 40 + 40, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            end
        end
    end
    ...
    function TRAIN_SYSTEM:ASNPScreen(Train)
        ...
        if State == 7 then
            ...
            local AnnLast = self.Train:GetNW2Bool("ASNP:AnnounceLast", false)
            if AnnLast then
                self:PrintText(0, 0, "ДО СТ")
            elseif Dep then
                self:PrintText(0, 0, "Отпр.")
            else
                self:PrintText(0, 0, "Приб.")
            end

            if AnnLast then
                self:PrintText(6, 0, En[2]:upper())
            else
...
function TRAIN_SYSTEM:Zero()
    self.Station = self.Path and self.LastStation or 1
    self.Arrived = true
    local tbl = Metrostroi.ASNPSetup[self.Train:GetNW2Int("Announcer", 1)][self.Line]
    if tbl then
        self.AnnounceLast = self.Path and self.FirstStation ~= 1 or not self.Path and self.LastStation ~= #tbl
    end
    self:UpdateBoards()
end
...
function TRAIN_SYSTEM:Next()
    ...
    if tbl.Loop then
        if self.Arrived then
            ...
            if self.Station == 0 or self.Station > #tbl then self.Station = self.Station == 0 and #tbl or 1 end
            if self.Station == 0 or self.Station > #tbl then self.Station = self.Station == 0 and (self.LastStation > 0 and self.LastStation or #tbl) or 1 end
            self.Arrived = false
            --self.Station = 1
        else
            self.Arrived = true
...
function TRAIN_SYSTEM:Prev()
    ...
    if tbl.Loop then
        ...
        if not self.Arrived then
            ...
            if self.Station ~= (self.Path and self.LastStation or self.FirstStation) then self.Arrived = false end
        end
    end
...
function TRAIN_SYSTEM:Play(dep, not_last)
    ...
    if dep then
        ...
        if lastst then
            ...
            if not message then message = stbl.arrlast[self.Path and 1 or 2] end
            if not message then message = stbl.arr[path] end
        else
...
function TRAIN_SYSTEM:SyncASNP()
    ...
    self.Train:CANWrite("ASNP", self.Train:GetWagonNumber(), "ASNP", nil, "FirstStation", not self.Path and 1 or self.FirstStation)
    self.Train:CANWrite("ASNP", self.Train:GetWagonNumber(), "ASNP", nil, "LastStation", self.Path and #tbl or self.LastStation)
...
function TRAIN_SYSTEM:UpdateBoards()
    ...
    if self.Path then
        for i = #stbl, self.Station + 1, -1 do
            if stbl[i] then curr = curr + stbl[i] end
        end
    else
...
function TRAIN_SYSTEM:Trigger(name, value)
    ...
    elseif self.State == 2 and value then
        ...
        if name == "R_ASNPMenu" then self.State = #stbl > 1 and 3 or 4 end
    ...
    elseif self.State == 3 and value then
        ...
        if name == "R_ASNPMenu" and value then
            if not tbl[self.Line].Loop then self.FirstStation = 1 end
            self.State = 4
        end
    elseif self.State == 4 and value and not tbl[self.Line].Loop then
        --Не кольцевой
        local stbl = Metrostroi.ASNPSetup[self.Train:GetNW2Int("Announcer", 1)][self.Line]
        if name == "R_ASNPDown" then self.Station = math.min(#stbl, self.Station + 1) end
        if name == "R_ASNPUp" then self.Station = math.max(1, self.Station - 1) end
    ...
    elseif self.State == 4 and value and tbl[self.Line].Loop then
        --Кольцевой
        if name == "R_ASNPDown" or name == "R_ASNPUp" then self.Path = not self.Path end
        if name == "R_ASNPMenu" then
            self.LastStation = 0
            self.FirstStation = 0
            self.State = 5
        end
    elseif self.State == 5 and value and not tbl[self.Line].Loop then
        --Не кольцевой
        local stbl = Metrostroi.ASNPSetup[self.Train:GetNW2Int("Announcer", 1)][self.Line]
        if name == "R_ASNPDown" then
            for i = math.min(#stbl, self.LastStation + 1), #stbl do
                if i ~= self.Station and stbl[i].arrlast then
                    self.LastStation = i
                    found = true
                    break
                end
    ...
    elseif self.State == 5 and value and tbl[self.Line].Loop then
        --Кольцевой
        local stbl = Metrostroi.ASNPSetup[self.Train:GetNW2Int("Announcer", 1)][self.Line]
        if name == "R_ASNPDown" then
            local found = false
            for i = self.LastStation + 1, #stbl do
                if stbl[i].arrlast then
                    self.LastStation = i
                    found = true
                    break
                end
            ...
            if not found and self.LastStation ~= 0 then self.LastStation = 0 end
        end
        ...
        if name == "R_ASNPUp" then
            ...
            for i = self.LastStation - 1, 1, -1 do
                if stbl[i].arrlast and stbl[i].arrlast[self.Path and 2 or 1] then
                    self.LastStation = i
                    found = true
                    break
                end
    ...
    elseif self.State == 6 and value then
        if name == "R_ASNPDown" or name == "R_ASNPUp" then self.Arrived = not self.Arrived end
        if name == "R_ASNPMenu" then
    ...
    elseif self.State == 7 then
        ...
        if name == "R_ASNPDown" and value and self.LineOut == 0 then self:Next() end
        if name == "R_ASNPUp" and value and self.LineOut == 0 then self:Prev() end
        if (name == "R_Program1" or name == "R_Program1H") and value and (not self.Train.R_Radio or self.Train.R_Radio.Value > 0) and self.LineOut ~= 0 then
            self:AnnQueue{-2, "buzz_end", "click2"}
        elseif (name == "R_Program1" or name == "R_Program1H") and value and (not self.Train.R_Radio or self.Train.R_Radio.Value > 0) and self.LineOut == 0 then
            ...
            if self.AnnounceLast then
                self.AnnounceLast = false
                self.PlayNextArmed = false
                local message = GetNotLastMessage(self)
                if message then
                    self:AnnQueue{"click1", "buzz_start"}
                    self:AnnQueue(message)
                    self:AnnQueue{"buzz_end", "click2"}
...
function GetNotLastMessage(self)
    local stbl = Metrostroi.ASNPSetup[self.Train:GetNW2Int("Announcer", 1)][self.Line]
    ...
    return ltbl_station.not_last_c and ltbl_station.not_last_c[self.Path and 2 or 1] and
        ltbl[ltbl_station.not_last_c[self.Path and 2 or 1]] or ltbl.not_last or nil
end
...
function TRAIN_SYSTEM:Think()
    ...
    if self.State == -1 and self.ASNPTimer and CurTime() - self.ASNPTimer > 1 then self.State = Metrostroi.ASNPSetup and #Metrostroi.ASNPSetup > 0 and 7 or -2 end
    ...
    if (not Metrostroi.ASNPSetup or Metrostroi.ASNPSetup and #Metrostroi.ASNPSetup == 0) and self.State > 0 then self.State = -2 end
    ...
    if self.PlayNextArmed and self.LineOut ~= LineOut and self.LineOut == 1 and LineOut == 0 then self:Next() end
    if self.TriggerButton and CurTime() > self.TriggerButtonTime + 0.2 and Train[self.TriggerButton].Value > 0 then self:Trigger(self.TriggerButton, true) end
    ...
    Train:SetNW2Bool("ASNP:AnnounceLast", self.AnnounceLast and GetNotLastMessage(self) and true or false)
    ...
    if Train.VBD and self.State > 0 then
        ...
        if Train.ALSCoil.Speed > 1 then
            ...
            if self.StopTimer and CurTime() - self.StopTimer >= 10 then self.StopTimer = false end
            local tbl = Metrostroi.ASNPSetup[self.Train:GetNW2Int("Announcer", 1)]
