...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_71_ASNP_OLD")
TRAIN_SYSTEM.DontAccelerateSimulation = true
function TRAIN_SYSTEM:Initialize()
    self.LineOut = 0
    self.TriggerNames = {"R_ASNPMenu", "R_ASNPUp", "R_ASNPDown", "R_ASNPOn", "R_Program1", "R_Program2", "R_Program1H", "R_Program2H",}
    --R_Announcer
    --R_Line
    self.Triggers = {}
    self.State = 0
    ...
    function TRAIN_SYSTEM:ASNPScreen(Train)
        ...
        if State == 2 then
            ...
            if sel == 2 then
                ...
                if timer == 1 then
                    self:PrintText(4, 1, "Для подтверждения")
                elseif timer == 2 then
                    self:PrintText(6, 1, "нажмите \"МЕНЮ\"")
                elseif timer == 3 then
                    self:PrintText(6, 1, "Для возврата к")
                elseif timer == 4 then
                    self:PrintText(5, 1, "предыдущей цифре")
                elseif timer == 5 then
                    self:PrintText(4, 1, "нажмите \"+\" или \"-\"")
                end
            else
                local timer = math.ceil(RealTime() % 7.5 / 1.5)
                if timer == 1 then
                    self:PrintText(5, 1, "Для ввода номера")
                elseif timer == 2 then
                    self:PrintText(4, 1, "нажмите \"+\" или \"-\"")
                elseif timer == 3 then
                    self:PrintText(6, 1, "Для перехода к")
                elseif timer == 4 then
                    self:PrintText(5, 1, "следующей цифре")
                elseif timer == 5 then
                    self:PrintText(6, 1, "нажмите \"МЕНЮ\"")
                end
            end
        ...
        if State == 3 then
            ...
            elseif timer == 2 then
                self:PrintText(0, 1, "От:")
                self:PrintText(3, 1, St[2])
                self:PrintText(21, 1, tostring(St[1]))
            elseif timer == 3 then
                self:PrintText(0, 1, "До:")
                self:PrintText(3, 1, En[2])
                self:PrintText(21, 1, tostring(En[1]))
            elseif timer == 4 then
                self:PrintText(4, 1, "Для подтверждения")
            elseif timer == 5 then
                self:PrintText(6, 1, "нажмите \"МЕНЮ\"")
        ...
        if State == 6 then
            ...
            if ltbl.Loop then
                ...
                if timer == 1 then
                    self:PrintText(0, 1, "()" .. (ltbl.Name or "Нет названия"))
                elseif timer == 2 and station > 0 then
                    self:PrintText(0, 1, "До:")
                    self:PrintText(3, 1, En[2])
                    self:PrintText(21, 1, tostring(En[1]))
                elseif timer == 2 and station == 0 then
                    self:PrintText(0, 1, "До:Кольцевой")
                    --elseif timer == 3 then self:PrintText(0,1,"До:");self:PrintText(3,1,En[2]);self:PrintText(21,1,tostring(En[1]))
                elseif timer == 3 then
                    self:PrintText(2, 1, "Для подтверждения")
                elseif timer == 4 then
                    self:PrintText(4, 1, "нажмите \"МЕНЮ\"")
                elseif timer == 5 then
                    self:PrintText(6, 1, "Для отмены")
                elseif timer == 6 then
                    self:PrintText(1, 1, "нажмите \"+\" или \"-\"")
                end
            else
                local St = ltbl[Train:GetNW2Int("ASNP:FirstStation", 1)]
                local En = ltbl[Train:GetNW2Int("ASNP:LastStation", 1)]
...
function TRAIN_SYSTEM:Trigger(name, value)
    ...
    elseif self.State == 2 and value then
        if name == "R_ASNPMenu" then
            ...
            if self.Selected > 2 then self.State = 3 end
        end
        ...
        if (name == "R_ASNPUp" or name == "R_ASNPDown") and self.Selected < 2 then
            ...
            if name == "R_ASNPUp" then
                if num == "9" then
                    self.RouteNumber = self.RouteNumber - 10 ^ sel * 9
                else
                    self.RouteNumber = self.RouteNumber + 10 ^ sel
                end
            end
    ...
    elseif self.State == 4 and value and not tbl[self.Line].Loop then
        ...
        if name == "R_ASNPMenu" then
            self.State = 5
            for i = #stbl, 1, -1 do
                if i ~= self.FirstStation and stbl[i].arrlast then
                    self.LastStation = i
                    break
                end
            end
    ...
    elseif self.State == 7 then
        ...
        if name == "R_ASNPMenu" and not value and self.ReturnTimer and self.ReturnTimer - CurTime() < 0.7 then self.ReturnTimer = nil end
        ...
        if (name == "R_Program1" or name == "R_Program1H") and value and self.LineOut == 0 then
            if self.Arrived and self.Station == (self.Path and self.FirstStation or self.LastStation) then self:Zero() end
...
function TRAIN_SYSTEM:Think()
    ...
    if self.State == -1 and self.ASNPTimer and CurTime() - self.ASNPTimer > 1 then self.State = Metrostroi.ASNPSetup and 1 or -2 end
    ...
    if not Metrostroi.ASNPSetup and self.State > 0 then self.State = -2 end
